    ...
    public class MaterialIcon : Label
    {
        ...
        public new class UxmlTraits : VisualElement.UxmlTraits
        {
            ...
            public override void Init(VisualElement ve, IUxmlAttributes bag, CreationContext cc)
            {
                ...
                target.IconName = icon.GetValueFromBag(bag, cc);

                // Set USS classes and text (codepoint of the icon) to render the icon from a Font Asset.
                target.AddToClassList("material-icon");
                if (string.IsNullOrEmpty(target.IconName))
                {
                    target.text = "";
                }
                else
                {
                    target.text = "\\u" + GetCodepointByIconName(target.IconName);
                }
            }
        }
        ...
        private static string GetCodepointByIconName(string iconName)
        {
                ...
                || iconNameToCodepoint.Count == 0)
            {
                ...
                foreach (string codepointLine in codepointLines)
                {
                    ...
                    {
                        string name = iconNameAndCodepoint[0];
                        string point = iconNameAndCodepoint[1];
                        iconNameToCodepoint[name] = point;
                    }
